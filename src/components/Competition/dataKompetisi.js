import React from "react";

const data = [
  {
    url: "yuk-visualkan",
    judul: "Yuk Visualkan !",
    tema: "Perguruan Tinggi Sebagai Bekal untuk Masa Depan",
    deskripsi: `"Yuk Visualkan" merupakan salah satu lomba yang diselenggarakan oleh Aku Masuk ITB 2021 berbentuk kreasi desain poster. Sasaran lomba ini adalah siswa- siswi SMA atau sederajat. Lomba ini diadakan sebagai wadah refleksi pentingnya pendidikan tinggi baik untuk diri sendiri maupun lingkungan sekitar.`,
    linkPendaftaran: "https://bit.ly/YukVisualkanAMI",
    ketentuanPeserta: (
      <ol>
        <li>
          Peserta merupakan pelajar SMA atau sederajat dan bersifat individu
        </li>
        <li>
          Peserta wajib membayar pendaftaran sebesar Rp 25.000,00 ke rekening
          90015104314 n Suci Ayu Chairuna Natalya (Jenius)
        </li>
        <li>
          Peserta wajib mengisi formulir pendaftaran (google form) yang tertera
          pada web
          <a href="http://www.akumasukitb.com/">
            <strong>akumasukitb.com</strong>
          </a>
        </li>
        <li>
          Peserta wajib menyiapkan dan mengumpulkan berkas dalam formulir
          pendaftaran, dengan ketentuan :
          <ol>
            <li>Scan Kartu Tanda Penduduk (KTP) atau Kartu Pelajar (.pdf)</li>
            <li>
              Surat Keterangan Siswa Aktif dengan kop dan cap resmi dari
              institusi / sekolah (.pdf)
            </li>
            <li>
              Lembar pernyataan orisinalitas yang ditandatangani di atas materai
              6000 (.pdf)
            </li>
            <li>Bukti Pembayaran (.jpg/jpeg)</li>
            <li>Foto formal berwarna ukuran 3x4 (.jpg / .jpeg)</li>
          </ol>
        </li>
        <li>
          Peserta wajib mengikuti akun Instagram Aku Masuk ITB 2021 di
          @akumasukitb
        </li>
        <li>
          Peserta yang memenuhi ketentuan pendaftaran akan diundang ke grup line
          dan berhak untuk mengumpulkan karyanya
        </li>
      </ol>
    ),
    ketentuanKarya: (
      <ol>
        <li>
          Poster yang dikumpulkan merupakan karya orisinil peserta yang belum
          pernah dipublikasikan, diikutsertakan dalam perlombaan lain, dan /atau
          digunakan untuk media komunikasi
        </li>
        <li>
          Poster dibuat semenarik, sekreatif, dan seinformatif mungkin dengan
          memperhatikan tema dan dapat memvisualkan pertanyaan "Kenapa harus
          melanjutkan pendidikan ke perguruan tinggi?"
        </li>
        <li>Teknis dan gaya pembuatan poster bebas</li>
        <li>
          Poster tidak boleh memuat unsur pornografi, kekerasan, dan menyinggung
          SARA (suku, agama, ras, dan antar golongan)
        </li>
        <li>Poster dibuat dengan software / aplikasi desain apapun</li>
        <li>
          Poster berukuran A3 (297 x 420 mm) dengan posisi diperbolehkan
          landscape atau portrait dengan format .png
        </li>
        <li>
          Peserta wajib membuat uraian singkat terdiri dari 100 - 200 kata yang
          mengandung deskripsi dan makna poster
        </li>
        <li>
          Uraian singkat poster ditulis dalam Bahasa Indonesia yang baik dan
          benar sesuai dengan PUEBI (Pedoman Umum Ejaan Bahasa Indonesia) dan
          diperbolehkan menggunakan singkatan yang menarik
        </li>
        <li>
          Pengumpulan karya yaitu berupa poster dan uraian singkat yang
          dikumpulkan melalui formulir google form yang telah disediakan
          panitia. Format penamaan file poster adalah "Nama Lengkap_Asal
          Sekolah_Yuk Visualkan.png"
        </li>
        <li>
          Poster yang dikumpulkan menjadi hak peserta dan panitia lomba Aku
          Masuk ITB 2021
        </li>
      </ol>
    ),
    ketentuanPenilaian: (
      <ol>
        <li>
          <strong>Tahap pertama (seleksi internal) :</strong>
          <ol>
            <li>
              Seleksi awal dilakukan panitia terkait ketentuan yang telah
              disebutkan sebelumnya.
            </li>
            <li>
              Seleksi berikutnya berdasarkan penilaian langsung oleh juri yang
              telah ditunjuk panitia dan menghasilkan 15 karya terbaik
            </li>
          </ol>
        </li>
        <li>
          <strong>Tahap kedua (ekshibisi) :</strong>
          <ol>
            <li>
              Ekshibisi dilaksanakan daring / online pada web akumasukitb.com
            </li>
            <li>
              Ekshibisi akan menampilkan poster dan uraian yang telah lolos
              seleksi sebelumnya.
            </li>
            <li>
              Setiap satu akun google memiliki satu suara dalam voting selama
              ekshibisi dibuka.
            </li>
          </ol>
        </li>
        <li>
          <strong>Pemenang :</strong>
          <ol>
            <li>
              Pemenang lomba ditentukan berdasarkan kriteria :
              <ul>
                <li>Makna poster (didapat pada poin 1b)</li>
                <li>Keindahan (didapat pada poin 1b)</li>
                <li>Votes ekshibisi online</li>
              </ul>
            </li>
            <li>
              Pemenang lomba akan mendapatkan sertifikat pemenang dan uang
              sebagai berikut :
            </li>
            <ul>
              <li>Juara 1 : Rp 1.500.000,00</li>
              <li>Juara 2 : Rp 1.250.000,00</li>
              <li>Juara 3 : Rp 750.000,00</li>
            </ul>
            <li>
              Pemenang akan diumumkan saat ITB DAY dan melalui kanal media
              sosial resmi Aku Masuk ITB 2021
            </li>
          </ol>
        </li>
        <li>
          <strong>Keputusan panitia dan juri tidak dapat diganggu gugat</strong>
        </li>
      </ol>
    ),
    liniMasa: (
      <table>
        <tbody>
          <tr>
            <th>No.</th>
            <th>Keterangan</th>
            <th>Waktu</th>
          </tr>
          <tr>
            <td>1</td>
            <td>Pendaftaran dan Verifikasi Peserta</td>
            <td>1 Desember – 20 Desember 2020</td>
          </tr>
          <tr>
            <td>2</td>
            <td>Pengumpulan Karya</td>
            <td>21 Desember – 23 Desember 2020</td>
          </tr>
          <tr>
            <td>3</td>
            <td>Seleksi Internal</td>
            <td>24 Desember, 26-27 Desember 2020</td>
          </tr>
          <tr>
            <td>4</td>
            <td>Pengumuman Seleksi Internal</td>
            <td>4 Januari 2021</td>
          </tr>
          <tr>
            <td>5</td>
            <td>Ekshibisi</td>
            <td>8 Januari – 10 Januari 2021</td>
          </tr>
          <tr>
            <td>6</td>
            <td>Pengumuman Pemenang</td>
            <td>6 Februari 2021</td>
          </tr>
        </tbody>
      </table>
    ),
    contactPerson: {
      nama: "Rayhan Muhammad Farras",
      line: "rayhan0013",
      whatsapp: "082244946946",
    },
  },
  {
    url: "check-the-mission",
    judul: "Check The Mission",
    tema: "Team up and Dare to Get Your Dream",
    deskripsi: `Lomba “Check The Mission” diselenggarakan oleh Aku Masuk ITB 2021 yang berbentuk permainan pos-to-pos interaktif menggunakan media website. Sasaran lomba ini adalah siswa-siswi SMA atau sederajat. Lomba ini diadakan untuk menyebarkan informasi perguruan tinggi secara umum dan ITB serta memacu sportivitas siswa.`,
    linkPendaftaran: "https://bit.ly/CheckMissionAMI",
    ketentuanPeserta: (
      <ol>
        <li>Peserta merupakan pelajar SMA atau sederajat</li>
        <li>
          Lomba ini bersifat kelompok. Setiap kelompok terdiri dari tiga orang.
          <br />
          (asal sekolah setiap anggota tidak diwajibkan sama)
        </li>
        <li>
          Peserta wajib membayar pendaftaran sebesar Rp30.000,00 ke rekening
          90015104314 a.n Suci Ayu Chairuna Natalya (Jenius)
        </li>
        <li>
          Peserta wajib mengisi formulir pendaftaran (google form) yang tertera
          pada web{" "}
          <a href="http://www.akumasukitb.com/">
            <strong>akumasukitb.com</strong>
          </a>
        </li>
        <li>
          Peserta wajib menyiapkan dan mengumpulkan berkas dalam formulir
          pendaftaran, dengan ketentuan :
          <ol>
            <li>Scan Kartu Tanda Penduduk (KTP) atau Kartu Pelajar (.pdf)</li>
            <li>
              Surat Keterangan Siswa Aktif dengan kop dan cap resmi dari
              institusi / sekolah (.pdf)
            </li>
            <li>Bukti pembayaran (.jpg/.jpeg)</li>
            <li>Foto formal berwarna ukuran 3x4 (.jpg/.jpeg)</li>
          </ol>
        </li>
        <li>
          Peserta wajib mengikuti akun Instagram Aku Masuk ITB 2021 di
          @akumasukitb
        </li>
        <li>
          Peserta yang memenuhi ketentuan pendaftaran akan diundang ke grup line
          dan berhak untuk lanjut ke tahap berikutnya
        </li>
      </ol>
    ),
    ketentuanKarya: (
      <ol>
        <li>Rangkaian lomba hanya diadakan dalam satu hari</li>
        <li>
          Tahap persiapan :
          <ol>
            <li>Seluruh peserta memasuki google meet yang telah ditentukan</li>
            <li>Panitia membacakan peraturan dan sistem lomba</li>
            <li>Pembagian urutan kelompok</li>
          </ol>
        </li>
        <li>
          Tahap pertama :
          <ol>
            <li>
              Seluruh peserta diarahkan membuka web lomba untuk menyelesaikan
              misi wajib untuk memperoleh poin awal
            </li>
            <li>
              Pada tahap ini, setiap kelompok akan diberikan suatu rangkaian
              huruf yang acak yang sama yang akan ditampilkan di web
            </li>
            <li>
              Peserta menyusun huruf acak tersebut menjadi kata baku yang sesuai
              dengan Kamus Besar Bahasa Indonesia
            </li>
            <li>
              Semakin banyak kata yang dapat dibuat, maka akan semakin banyak
              jumlah poin yang akan didapatkan
            </li>
          </ol>
        </li>
        <li>
          Tahap kedua :
          <ol>
            <li>
              Seluruh peserta meninggalkan google meet utama. Kemudian membuka
              peta yang telah tersedia di web.
            </li>
            <li>
              Terdapat lima pos di dalam peta yang berisi link google meet dan
              peraturan untuk masing-masing pos
            </li>
            <li>
              Para peserta memasuki empat pos pertama sesuai urutan yang telah
              ditentukan di tahap persiapan
            </li>
            <li>
              Setiap pos memiliki durasi 20 menit. Bila peserta berhasil
              menyelesaikan pos, maka akan mendapatkan kode unik yang digunakan
              untuk memasuki pos lima
            </li>
            <li>
              Keterangan lebih lanjut mengenai pos :
              <ul>
                <li>
                  POS 1 : Tebak Gambar
                  <br />
                  Tema : Alumni ITB yang berpengaruh di Indonesia
                  <br />
                  Peserta akan menebak potongan gambar terkait tema pos
                  <br />
                  <br />
                </li>
                <li>
                  POS 2 : Benar atau Salah
                  <br />
                  Tema : ITB dan Kegiatan Kemahasiswaannya.
                  <br />
                  Peserta akan menebak mana saja pertanyaan yang benar atau
                  salah terkait tema pos
                  <br />
                  <br />
                </li>
                <li>
                  POS 3 : Menyusun Puzzle Tema : Lokasi dan Gedung ITB
                  <br />
                  Peserta akan menyusun puzzle berupa potongan gambar terkait
                  tema pos.
                  <br />
                  <br />
                </li>
                <li>
                  POS 4 : Teka Teki Silang
                  <br />
                  Tema : Jurusan dan beasiswa di ITB
                  <br />
                  Peserta akan mengisi dan menjawab pertanyaan pada teka teki
                  silang terkait tema pos.
                  <br />
                  <br />
                </li>
                <li>
                  POS 5 : Test Your brain
                  <br />
                  Tema : Jalur Masuk dan Jenis Perguruan Tinggi di Indonesia.
                  <br />
                  Setiap kelompok akan menjawab pertanyaan dengan media quizizz
                  terkait tema pos yang dikerjakan serentak oleh semua kelompok
                </li>
              </ul>
            </li>
          </ol>
        </li>
      </ol>
    ),
    ketentuanPenilaian: (
      <ol>
        <li>
          Tahap pertama :
          <ol>
            <li>
              Setiap kata yang ditemukan dan benar sesuai dengan KBBI diberikan
              poin 10 per kata
            </li>
            <li>
              Perolehan poin total akan dijadikan poin modal awal dalam memulai
              misi pos selanjutnya
            </li>
          </ol>
        </li>
        <li>
          Tahap kedua :
          <ol>
            <li>
              POS 1, 2, dan 4<br />
              Benar (+5), salah (-2), tidak menjawab (0)
            </li>
            <li>
              POS 3<br />
              Letak potongan puzzle benar (+5), salah / tidak meletakkan
              potongan puzzle (0).
            </li>
            <li>
              POS 5<br />
              Sistem poin berbeda setiap soal
            </li>
          </ol>
        </li>
        <li>
          Pemenang :
          <ol>
            <li>
              Pemenang lomba ditentukan berdasarkan perolehan poin tertinggi
              yang didapat dari awal sampai akhir misi
            </li>
            <li>
              Pemenang lomba akan mendapatkan sertifikat digital pemenang dan
              uang sebagai berikut :
              <ul>
                <li>Juara 1 : Rp1.500.000,00</li>
                <li>Juara 2 : Rp1.200.000,00</li>
                <li>Juara 3 : Rp1.000.000,00</li>
              </ul>
            </li>
            <li>
              Pemenang akan diumumkan saat ITB DAY dan melalui kanal media
              sosial resmi Aku Masuk ITB 2021
            </li>
          </ol>
        </li>
        <li>
          <strong>Keputusan panitia dan juri tidak dapat diganggu gugat</strong>
        </li>
      </ol>
    ),
    liniMasa: (
      <table>
        <tbody>
          <tr>
            <th>No.</th>
            <th>Keterangan</th>
            <th>Waktu</th>
          </tr>
          <tr>
            <td>1.</td>
            <td>Pendaftaran</td>
            <td>1 Desember – 27 Desember 2020</td>
          </tr>
          <tr>
            <td>2.</td>
            <td>Verifikasi Peserta</td>
            <td>15 Desember – 27 Desember 2020</td>
          </tr>
          <tr>
            <td>3.</td>
            <td>Technical Meeting Virtual</td>
            <td>9 Januari - 10 Januari 2021</td>
          </tr>
          <tr>
            <td>4.</td>
            <td>Pelaksanaan lomba</td>
            <td>24 Januari 2021</td>
          </tr>
          <tr>
            <td>6.</td>
            <td>Pengumuman Pemenang</td>
            <td>6 Februari 2021</td>
          </tr>
        </tbody>
      </table>
    ),
    contactPerson: {
      nama: "Nabila Salma Nareswari (Salma)",
      line: "nabilasalmon",
      whatsapp: "081510430650",
    },
  },
  {
    url: "musikalisasi-puisi",
    judul: "Lomba Musikalisasi Puisi",
    tema: "Janjiku Kepada Bangsa Indonesia",
    deskripsi: `Lomba Musikalisasi Puisi merupakan salah satu lomba yang diselenggarakan oleh Aku Masuk ITB 2021. Sasaran lomba ini adalah siswa-siswi SMA atau sederajat. Lomba ini diadakan sebagai wadah eksplorasi potensi diri untuk menyokong kemajuan bangsa.`,
    linkPendaftaran: "https://bit.ly/MusPusAMI",
    ketentuanPeserta: (
      <ol>
        <li>Peserta merupakan pelajar SMA atau sederajat</li>
        <li>
          Lomba ini bersifat kelompok. Setiap kelompok terdiri dari tiga sampai
          lima orang. (asal sekolah setiap anggota tidak diwajibkan sama)
        </li>
        <li>
          Peserta wajib membayar pendaftaran sebesar Rp 30.000,00 ke rekening
          90015104314 n Suci Ayu Chairuna Natalya (Jenius)
        </li>
        <li>
          Peserta wajib mengisi formulir pendaftaran (google form) yang tertera
          pada web{" "}
          <a href="http://www.akumasukitb.com/">
            <strong>akumasukitb.com</strong>
          </a>
        </li>
        <li>
          Peserta wajib menyiapkan dan mengumpulkan berkas dalam formulir
          pendaftaran, dengan ketentuan :
          <ol>
            <li>Scan Kartu Tanda Penduduk (KTP) atau Kartu Pelajar (.pdf)</li>
            <li>
              Surat Keterangan Siswa Aktif dengan kop dan cap resmi dari
              institusi/sekolah (.pdf)
            </li>
            <li>
              Lembar pernyataan orisinalitas yang ditandatangani di atas materai
              6000 (.pdf)
            </li>
            <li>Bukti Pembayaran (.jpg/jpeg)</li>
            <li>Foto formal berwarna ukuran 3x4 (.jpg / .jpeg)</li>
          </ol>
        </li>
        <li>
          Peserta wajib mengikuti akun Instagram Aku Masuk ITB 2021 di
          @akumasukitb
        </li>
        <li>
          Peserta yang memenuhi ketentuan pendaftaran akan diundang ke grup line
          dan berhak untuk mengumpulkan karyanya
        </li>
      </ol>
    ),
    ketentuanKarya: (
      <ol>
        <li>
          Puisi yang akan dimusikalisasi merupakan karya orisinil peserta yang
          belum pernah dipublikasikan, diikutsertakan dalam perlombaan lain, dan
          /atau digunakan untuk media komunikasi
        </li>
        <li>
          Puisi dan video musikalisasi dibuat semenarik, dan sekreatif mungkin
          dengan memperhatikan tema
        </li>
        <li>
          Puisi dan video musikalisasi tidak boleh memuat unsur pornografi,
          kekerasan, dan menyinggung SARA (suku, agama, ras, dan antar golongan)
        </li>
        <li>
          Puisi dinyanyikan secara utuh (tidak dideklamasikan atau didramatisasi)
        </li>
        <li>
          Jika terdapat pengulangan, puisi dinyanyikan secara utuh, tidak hanya sebagian.
        </li>
        <li>
          Genre musikalisasi puisi dibebaskan sesuai dengan genre musik yang ada.
        </li>
        <li>
          Video musikalisasi puisi dibuat dengan durasi maksimal 7 menit (tanpa
          pembukaan dan penutupan)
        </li>
        <li>
          Peserta diperkenankan memotong dan/atau menyunting audio dan video
        </li>
        <li>
          Video musikalisasi puisi harus menampilkan semua anggota kelompok
          tetapi tidak diharuskan berada pada satu tempat yang sama
        </li>
        <li>
          Peserta wajib mengunggah video musikalisasi puisi ke google drive
          masing- masing.
        </li>
        <li>
          Karya dikumpulkan dalam bentuk dokumen puisi (.pdf) dan juga link
          video musikalisasi. Kedua komponen itu dikumpulkan melalui formulir
          pengumpulan google form yang akan diberikan panitia
        </li>
        <li>
          Peserta wajib mengunggah video musikalisasi puisi ke IGTV di akun
          Instagram salah satu anggota kelompok pada waktu yang ditentukan
          dengan menandai akun resmi Instagram Aku Masuk ITB 2021
        </li>
        <li>
          Puisi dan video musikalisasi yang dikumpulkan menjadi hak peserta dan
          panitia lomba Aku Masuk ITB 2021
        </li>
      </ol>
    ),
    ketentuanPenilaian: (
      <ol>
        <li>
          <strong>Kriteria Penilaian :</strong>
          <ol>
            <li>
              Penafsiran puisi
              <br />
              <ul>
                <li>Isi puisi</li>
                <li>Kesesuaian dengan tema</li>
                <li>Pemilihan kata dan penggunaan diksi</li>
              </ul>
            </li>
            <li>
              Harmonisasi dan keselarasan
              <ul>
                <li>Keselarasan bunyi, baik vokal maupun alat musik</li>
                <li>Kemampuan menyusun suatu komponen music menjadi satu</li>
              </ul>
            </li>
            <li>
              Vokal
              <ul>
                <li>Kualitas suara</li>
                <li>Intonasi suara</li>
                <li>Artikulasi</li>
              </ul>
            </li>
            <li>
              Penampilan
              <ul>
                <li>Kostum</li>
                <li>Gestur dan ekspresi</li>
                <li>Tata rias</li>
              </ul>
            </li>
            <li>
              Editing video
              <br />
              <br />
            </li>
          </ol>
        </li>
        <li>
          <strong>Pemenang :</strong>
          <ol>
            <li>
              Pemenang lomba adalah kelompok yang memiliki poin tertinggi
              berdasar kriteria penilaian
            </li>
            <li>
              Pemenang lomba akan mendapatkan sertifikat digital pemenang, dan
              uang sebagai berikut :
              <ul>
                <li>Juara 1 : Rp1.500.000,00</li>
                <li>Juara 2 : Rp1.200.000,00</li>
                <li>Juara 3 : Rp 1.000.000,00</li>
              </ul>
            </li>
            <li>
              Pemenang akan diumumkan saat ITB DAY dan melalui kanal media
              sosial resmi AKU MASUK ITB 2021
            </li>
            <li>
              <strong>
                Keputusan panitia dan juri tidak dapat diganggu gugat
              </strong>
            </li>
          </ol>
        </li>
      </ol>
    ),
    liniMasa: (
      <table>
        <tbody>
          <tr>
            <th>No.</th>
            <th>Keterangan</th>
            <th>Waktu</th>
          </tr>
          <tr>
            <td>1.</td>
            <td>Pendaftaran dan Verifikasi Peserta</td>
            <td>1 Desember – 27 Desember 2020</td>
          </tr>
          <tr>
            <td>2.</td>
            <td>Pengerjaan Karya</td>
            <td>22 Desember 2020 – 10 Januari 2021</td>
          </tr>
          <tr>
            <td>3.</td>
            <td>Pengumpulan Karya</td>
            <td>1 Januari – 10 Januari 2021</td>
          </tr>
          <tr>
            <td>4.</td>
            <td>Pengunggahan karya ke IGTV</td>
            <td>11 Januari - 15 Januari 2021</td>
          </tr>
          <tr>
            <td>5.</td>
            <td>Penilaian Lomba</td>
            <td>1 Januari – 15 Januari 2021</td>
          </tr>
          <tr>
            <td>6.</td>
            <td>Pengumuman Pemenang</td>
            <td>6 Februari 2021</td>
          </tr>
        </tbody>
      </table>
    ),
    contactPerson: {
      nama: "Rafael Rubens",
      line: "grubens",
      whatsapp: "085659398820",
    },
  },
  {
    url: "komik-karikatur",
    judul: "Komik Karikatur",
    tema: "Buah Manis Dari Pohon Pendidikan",
    deskripsi: `"Komik Karikatur” merupakan salah satu lomba yang diselenggarakan oleh Aku Masuk ITB 2021 berbentuk komik dengan gaya menggambar karikatural. Sasaran lomba ini adalah siswa-siswi SMA atau sederajat. Lomba ini diadakan untuk mendorong rasa berkeinginan melanjutkan pendidikan ke tahap perguruan tinggi.`,
    linkPendaftaran: "https://bit.ly/KarikaturKomikAMI",
    ketentuanPeserta: (
      <ol>
        <li>
          Peserta merupakan pelajar SMA atau sederajat dan bersifat individu
        </li>
        <li>
          Peserta wajib membayar pendaftaran sebesar Rp 25.000,00 ke rekening
          90015104314 n Suci Ayu Chairuna Natalya (Jenius)
        </li>
        <li>
          Peserta wajib mengisi formulir pendaftaran (google form) yang tertera
          pada web{" "}
          <a href="http://www.akumasukitb.com/">
            <strong>akumasukitb.com</strong>
          </a>
        </li>
        <li>
          Peserta wajib menyiapkan dan mengumpulkan berkas dalam formulir
          pendaftaran, dengan ketentuan :
          <ol>
            <li>Scan Kartu Tanda Penduduk (KTP) atau Kartu Pelajar (.pdf)</li>
            <li>
              Surat Keterangan Siswa &nbsp;Aktif &nbsp;dengan &nbsp;kop
              &nbsp;dan &nbsp;cap &nbsp;resmi &nbsp;dari &nbsp;institusi /
              sekolah (.pdf)
            </li>
            <li>
              Lembar pernyataan orisinalitas yang ditandatangani di atas materai
              6000 (.pdf)
            </li>
            <li>Bukti pembayaran (.jpg/.jpeg)</li>
            <li>Foto formal berwarna ukuran 3x4 (.jpg / .jpeg)</li>
          </ol>
        </li>
        <li>
          Peserta wajib mengikuti akun Instagram Aku Masuk ITB 2021 di
          @akumasukitb
        </li>
        <li>
          Peserta yang memenuhi ketentuan pendaftaran akan diundang ke grup line
          dan berhak untuk mengumpulkan karyanya
        </li>
      </ol>
    ),
    ketentuanKarya: (
      <ol>
        <li>
          Komik karikatur yang dikumpulkan merupakan karya orisinil peserta yang
          belum pernah dipublikasikan, diikutsertakan dalam perlombaan lain, dan
          /atau digunakan untuk media
        </li>
        <li>
          Komik karikatur dibuat semenarik, sekreatif, dan seinformatif mungkin
          dengan memperhatikan tema dan dapat memvisualkan pertanyaan{" "}
          <strong>
            “Manfaat apa yang kita dapat dari berpendidikan tinggi?”
          </strong>
        </li>
        <li>
          Teknis dan gaya pembuatan komik karikatur dengan metode penggambaran
          karikatural secara manual
        </li>
        <li>
          Komik karikatur tidak boleh memuat unsur pornografi, kekerasan, dan
          menyinggung SARA (suku, agama, ras, dan antar golongan)
        </li>
        <li>
          Pada tahap 1, karya yang dikumpulkan berupa sketsa (belum diwarnai);
          Pada tahap 2 karya yang dikumpulkan berupa komik karikatur berwarna
        </li>
        <li>
          Komik karikatur berukuran A3 (297 / 420mm) dengan posisi diperbolehkan
          landscape atau portrait
        </li>
        <li>
          Hasil karya yang dikumpulkan meliputi :
          <ol>
            <li>Scan Karya Full (A3) (.pdf)</li>
            <li>Scan karya setengah bagian (A3 dibagi 2 menjadi A4) (.pdf)</li>
            <li>
              Scan karya setengah bagian lainnya (A3 dibagi 2 menjadi A4) (.pdf)
            </li>
          </ol>
        </li>
        <li>
          Pengumpulan karya yaitu berupa komik karikatur yang dikumpulkan
          melalui formulir google form yang telah disediakan panitia. Format
          penamaan file komik karikatur adalah AMI_No Peserta_Judul.pdf.
        </li>
        <li>
          Komik karikatur yang dikumpulkan menjadi hak peserta dan panitia lomba
          Aku Masuk ITB 2021.
        </li>
      </ol>
    ),
    ketentuanPenilaian: (
      <ol>
        <div>
          <strong>1.&nbsp; Tahap pertama (seleksi internal) :</strong>
          <ol>
            <li>
              Seleksi awal dilakukan panitia terkait ketentuan yang telah
              disebutkan sebelumnya.
            </li>
            <li>
              Seleksi berikutnya berdasarkan penilaian langsung oleh dua juri
              yang telah ditunjuk panitia dan menghasilkan 8 karya
            </li>
          </ol>
        </div>
        <div>
          <strong>2.&nbsp; Tahap kedua (ekshibisi) :</strong>
          <ol>
            <li>
              Ekshibisi dilaksanakan daring / online pada web{" "}
              <a href="http://www.akumasukitb.com/">
                <strong>akumasukitb.com</strong>
              </a>
            </li>
            <li>
              Ekshibisi akan menampilkan komik karikatur yang telah lolos eleksi
              sebelumnya.
            </li>
            <li>
              Setiap satu akun google memiliki satu suara dalam voting selama
              ekshibisi dibuka.
            </li>
          </ol>
        </div>
        <div>
          <strong>3.&nbsp; Pemenang :</strong>
          <ol>
            <div>
              <p>Pemenang lomba ditentukan berdasarkan kriteria :</p>
              <ul>
                <li>Kesesuaian Tema</li>
                <li>Alur dan narasi cerita</li>
                <li>Keindahan dari komik karikatur</li>
                <li>Votes ekshibisi online</li>
              </ul>
            </div>
            <div>
              <p>
                Pemenang lomba akan mendapatkan sertifikat pemenang dan uang
                sebagai berikut :
              </p>
              <ul>
                <li>&nbsp;Juara 1 : Rp 1.500.000,00</li>
                <li>&nbsp;Juara 2 : Rp 1.250.000,00</li>
                <li>&nbsp;Juara 3 : Rp 750.000,00</li>
              </ul>
            </div>
          </ol>
        </div>
        <div>
          <strong>
            4.&nbsp; Pemenang akan diumumkan saat ITB DAY dan melalui kanal
            media sosial resmi Aku Masuk ITB 2021
          </strong>
        </div>
        <div>
          <strong>
            5.&nbsp; Keputusan panitia dan juri tidak dapat diganggu gugat
          </strong>
        </div>
      </ol>
    ),
    liniMasa: (
      <table>
        <tbody>
          <tr>
            <th>No.</th>
            <th>Keterangan</th>
            <th>Waktu</th>
          </tr>
          <tr>
            <td>1.</td>
            <td>Pendaftaran dan Verifikasi Peserta</td>
            <td>1 Desember – 20 Desember 2020</td>
          </tr>
          <tr>
            <td>2.</td>
            <td>Pengumpulan Karya tahap 1</td>
            <td>7 Desember – 27 Desember 2020</td>
          </tr>
          <tr>
            <td>3.</td>
            <td>Seleksi Tahap 1</td>
            <td>23 Desember 2020 - 3 Januari 2021</td>
          </tr>
          <tr>
            <td>4.</td>
            <td>Pengumuman Seleksi Tahap 1</td>
            <td>4 Januari 2021</td>
          </tr>
          <tr>
            <td>5.</td>
            <td>Pengumpulan Karya Tahap 2</td>
            <td>8 Januari – 20 Januari 2021</td>
          </tr>
          <tr>
            <td>6.</td>
            <td>Ekshibisi Online dan Voting</td>
            <td>29 Januari - 31 Januari 2021</td>
          </tr>
          <tr>
            <td>7.</td>
            <td>Seleksi Tahap 2</td>
            <td>20 Januari - 5 Februari 2021</td>
          </tr>
          <tr>
            <td>8.</td>
            <td>Pengumuman Pemenang</td>
            <td>6 Februari 2021</td>
          </tr>
        </tbody>
      </table>
    ),
    contactPerson: {
      nama: "Sherina Fitri Agustin",
      line: "seriiin_",
      whatsapp: "083113329550",
    },
  },
];

export default data;
